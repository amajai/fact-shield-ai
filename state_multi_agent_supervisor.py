...
class ConductResearch(BaseModel):
    """Tool for delegating a fact-check task to a specialized sub-agent."""
    research_topic: str = Field(
        description="The topic to fact-check. Should be a single topic, and should be described in high detail (at least a paragraph).",
    )

@tool
class ResearchComplete(BaseModel):
    """Tool for indicating that the fact-check process is complete."""
    pass
